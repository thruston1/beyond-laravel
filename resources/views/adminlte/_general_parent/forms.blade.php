    ...
    <section class="content">
        <div class="container-fluid">
            <div class="card {!! $printCard !!}">
                ...
                @if($viewType == 'create')
                    {{ Form::open(['route' => ['admin.' . $thisRoute . '.store', $parentId], 'files' => true, 'id'=>'form', 'role' => 'form'])  }}
                @elseif($viewType == 'edit')
                    {{ Form::open(['route' => ['admin.' . $thisRoute . '.update', $parentId, $data->{$masterId}], 'method' => 'PUT', 'files' => true, 'id'=>'form', 'role' => 'form'])  }}
                @else
                ...
                <div class="card-footer">
                    ...
                    @elseif ($viewType == 'show' && $permission['edit'] == true)
                        <a href="<?php echo route('admin.' . $thisRoute . '.edit', [$parentId, $data->{$masterId}]) ?>"
                           class="mb-2 mr-2 btn btn-primary" title="{{ __('general.edit') }}">
                            <i class="fas fa-pencil"></i><span class=""> {{ __('general.edit') }}</span>
                        </a>
                    @endif
                    <a href="<?php echo route('admin.' . $thisRoute . '.index', $parentId) ?>" class="mb-2 mr-2 btn btn-warning"
